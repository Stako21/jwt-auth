...
- Integration rejection through `POST /api/1c/tro-requests/:id/rejected` requires a GUID/name snapshot of the real 1C responsible. Store it only in `tro_document_details.rejected_by_1c_guid` / `rejected_by_1c_name`, preserve the technical integration account in `document_history.user_id`, and keep `executor_*`, `author_user_id`, and workflow `ta_user_id` unchanged. Identical retries compare reason, reason code, responsible GUID, and responsible name; conflicting retries must remain `409`.
