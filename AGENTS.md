...
- The Documents list defaults to the current calendar month, supports independently unbounded start/end dates, and returns every document matching the selected period. Its document workspace provides an always-visible status checkbox list, contractor typeahead matching the creation modals, and author filtering; desktop uses a dense bounded table with icon-only status tooltips and a fixed action area, while phones use compact cards with visible icon-plus-text status labels and touch-sized workflow/history/PDF actions. Signed status icon/text remains semantic green across every palette. Return/exchange creation and editing modals use the same token-driven visual system, compact controls, highlighted executor/item/footer blocks, scroll-contained item tables, and mobile bottom-sheet layouts; field order and document workflow remain unchanged. Desktop modal cards are centered in the live content area after the current 280px/72px sidebar rather than across the full viewport.
