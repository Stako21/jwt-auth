...
- Both document workspaces support multi-selection from desktop rows and mobile cards. The toolbar outside the results scroller selects all currently filtered documents, clears selection, prints a compact landscape registry, or downloads an XLSX registry. Registry endpoints accept only explicit selected IDs, revalidate access to every document server-side, reject cross-workspace selections, and use one shared normalized column model for print and XLSX. TRO and return/exchange registries keep product names and quantities in separate adjacent columns, with multi-item values aligned line by line; exchange product names retain their take/give operation context.
