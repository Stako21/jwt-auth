...
- The current frontend shell uses the Stitch-aligned `Obsidian Emerald Executive` dark operational design: all authored frontend color literals and semantic palette values are centralized in `client/src/variables.scss`, which also exports the shared `--ui-*` runtime tokens and Bulma compatibility mappings. Component SCSS/CSS/JS must consume those tokens rather than introducing local `hex`/`rgb`/`hsl` or named colors. The shell includes a collapsible glass desktop sidebar/compact topbar, a mobile drawer, and matching login/balance surfaces with 8px controls and 16px cards. Bulma remains only as a transitional compatibility layer and is loaded from the installed package rather than a CDN.
- Four structurally compatible theme token sets are available: emerald dark in `variables.scss`, cyan dark in `variables-cyan-dark.scss`, emerald light in `variables-emerald-light.scss`, and cyan light in `variables-cyan-light.scss`. Each exposes the same SCSS token contract and runtime/Bulma mixins; automatic runtime theme selection is not implemented yet. Keep all four contracts synchronized when adding or renaming a token.
