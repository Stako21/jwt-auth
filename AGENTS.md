...
- Balance XLSX pages support both the legacy automatic parser and an optional validated column contract in `balance_pages.header_row`, `header_end_row`, `data_start_row`, and `column_config`. Admin can configure a one-row or multi-row header range, inspect the current workbook, select/reorder/rename output columns, designate exactly one hierarchy column, optionally designate one displayed price column for `price_multiplier_percent`, and preview rows before saving. Multi-row source identities are built from the full vertical header path and expand Excel merged cells before matching. Configured pages validate saved header snapshots against the current workbook and must show an explicit structure-mismatch error instead of silently reading shifted columns. Legacy pages without `column_config` keep automatic detection and the existing price popover behavior.
