...
- Table surfaces across document lists, balances, modal item grids, reports, user/admin screens, and remaining Bulma compatibility tables share the `ReportXlsx1C` charcoal/emerald palette through centralized `$table-*` / `--ui-table-*` tokens. Semantic totals, hierarchy levels, warnings, errors, and workflow states may override the base row colors.
...
- Use the centralized table palette tokens for new table shells, headers, borders, alternating rows, text, group rows, and hover states; do not introduce a separate generic table palette in individual report modules.
