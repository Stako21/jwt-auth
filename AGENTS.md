...
- Four structurally compatible runtime theme token sets are available: emerald dark in `variables.scss`, cyan dark in `variables-cyan-dark.scss`, emerald light in `variables-emerald-light.scss`, and cyan light in `variables-cyan-light.scss`. Component styles consume their colors through the `client/src/theme.scss` runtime facade, while geometry and motion remain compile-time Sass tokens. Admin exposes a browser-local theme selector; `data-ui-theme` selects the palette and the standard `data-theme` stays synchronized to `dark`/`light` for Bulma/native controls. Keep all four contracts and their `export-runtime-theme-colors` mixins synchronized when adding or renaming a color token.
