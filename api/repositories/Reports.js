...
class ReportsRepository {
  ...
  static async getVisibleTaUsers({ userId, role, branchId }) {
    ...
    if (normalizedRole === ROLE_IDS.TA) {
      sql = `
        ...
        WHERE u.id = :current_user_id
          ...
          AND u.role = ${ROLE_IDS.TA}
      `;
    } else if (normalizedRole === ROLE_IDS.SV) {
      sql = `
        SELECT
          u.id,
          u.name AS login,
      ...
      `;
    } else if (normalizedRole === ROLE_IDS.NTO) {
      sql = `
        WITH RECURSIVE user_tree AS (
          SELECT child_user_id
          FROM user_hierarchy
        ...
        WHERE u.branch_id = :branch_id
          ...
          AND u.role = ${ROLE_IDS.TA}
          AND (
            u.id IN (SELECT child_user_id FROM user_tree)
            OR h.parent_user_id IS NULL
          )
      `;
    } else if (normalizedRole === ROLE_IDS.Accountant) {
      sql = `
        SELECT
          u.id,
          u.name AS login,
